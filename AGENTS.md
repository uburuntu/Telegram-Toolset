...
- `CONTRIBUTING.md` is the contributor workflow and tool checklist; `SECURITY.md` is the private
  vulnerability-reporting contract; `CHANGELOG.md` distinguishes the current web app from legacy
  desktop releases.
...
- `public/logo.png` is the primary brand mark and `public/favicon.png` is its small monochrome
  variant. `public/social-preview.png` is generated from the primary mark by
  `scripts/generate-social-preview.mjs`; edit the source and run `npm run social:generate` rather
  than hand-editing the social image.
